...
if($sort=='info'){$url=$资源接口.'?ac=videolist&ids='.$id;}elseif($sort=='search'){$url=$资源接口.'?ac=videolist&wd='.$id.'&pg='.$page;}else{
    ...
    global $倒序; if($倒序=='yes'){$page=$pagz-$page;}
...
$过度=json_decode($数据,TRUE);$过度=$过度['list'][video];
...
if($sort=='list'){
    ...
    if(empty($当前分类名)){$当前分类名='最近更新';$当前分类号='';}
