...
var BatchTasker={
    ...
    BeginBatch:function(ci,pid){
        ...
        if(this.isIE){
            if(this.Agent == undefined || this.Agent == null){
                //alert(Thunder5SetUpInfo);
                //top.location.href= wtd_ChangFolder(pid?pid:0);
                pluginMissingTips(pid);
                return false;
            }
            ...
            if(this.Agent == undefined || this.Agent == null){
                if(navigator.mimeTypes["application/np_xunlei_plugin"] || navigator.mimeTypes["application/np_xunlei_plugin.2"]){
                    return true;
                }else{
                    //alert(Thunder5SetUpInfo);
                    //top.location.href= wtd_ChangFolder(pid?pid:0);
    ...
    AddTask:function(txtUrl,urlName){
        ...
        if(this.Agent == undefined || this.Agent == null) this.initialize();
        if(this.isIE){
            this.Agent.AddBatchTask(txtUrl,urlName);
        }else{
            if(this.Agent == undefined || this.Agent == null){
                if(navigator.mimeTypes["application/np_xunlei_plugin.2"]){//批量接口
                    thunderBatchTasker.AddBatchTask(txtUrl, urlName);
                    //thunderBatchTasker.tasks[thunderBatchTasker.tasks.length]=new Array(txtUrl,'');
                }
                else if(navigator.mimeTypes["application/np_xunlei_plugin"]){
                    DownloadByThunder(txtUrl, this.pid);
                }
            }else{
                netscape.security.PrivilegeManager.enablePrivilege("UniversalXPConnect");
        ...
        if(this.isIE){
            ...
            if(this.Agent == undefined || this.Agent == null){
                ...
                if (npXLPlugin2) 
                {
                    ...
                    var g_strSplitter = "#@$@#";
                    // ReferUrl（引用页，当前网页）
                    var strUrls = document.location.href;
                    strUrls = strUrls.concat(g_strSplitter);
                    // LinkCount（任务个数）
                    strUrls = strUrls.concat(thunderBatchTasker.tasks.length, g_strSplitter);
                    for(var i = 0; i < thunderBatchTasker.tasks.length; i ++){
                            // LinkUrl
                            strUrls = strUrls.concat(thunderBatchTasker.tasks[i][0], g_strSplitter);
                            // filename 当filename与url相同时，置为空
                            if(thunderBatchTasker.tasks[i][0] == thunderBatchTasker.tasks[i][1]){
                                strUrls = strUrls.concat('', g_strSplitter);
                            }
                            else{
                                strUrls = strUrls.concat(thunderBatchTasker.tasks[i][1], g_strSplitter);
                            }
    ...
    initialize:function(){
        ...
        if(this.isIE){
            ...
            try{
                ...
                //alert("error: this.Agent is null");
                this.Agent = null;
                return;
            }
        }
...
function quanxuan()
{
    var items= document.getElementsByName("enjoy[]");
...
function thunderLinker()
{
    ...
    try{
        htmlB=thunderHTMLInsert;
    }catch (e){
        htmlB="&nbsp;";
    }
    try{
        convertCount=thunderLinkCount;
    }catch(e){
        convertCount=0;
    }
    try{
        exceptPath=thunderExceptPath;
    }catch (e){
        exceptPath="";
    }
    try{
        linkModal=thunderLinkHTML
        }catch (e){}
    ...
    {
        try{
            linkStyle=thunderLinkStyle;
        }catch(e){
            linkStyle="";
        }
    }
    if(linkStyle!=""){
        linkStyle=linkStyle.replace(/\\/g,"\\")
        }
    var thunderSufix=".doc|.chm|.asf|.avi|.exe|.iso|.mp3|.mpeg|.mpg|.mpga|.ra|.rar|.rm|.rmvb|.tar|.wma|.wmv|.zip|.swf|.mp4|.3gp|.torrent|.txt|.jar|.mov|.wav|.eip";
...
//var thunderPid="97379";
//var thunderPath=/\/attachment\.php\?/gi;