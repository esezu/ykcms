...
{
	ajax.get( 
		"/"+sitePath+"duomiphp/ajax.php?action=addfav&id="+vid+"&uid="+uid,
		function(obj) {
			if (obj.responseText=="err"){
				alert("请先登录!");
				location.href="/"+sitePath+"member/login.php";
			}else{
				alert("收藏成功!");
			}
		}
...
function viewComment(id,page){
	var url;
	if (page.length==0){url=id;}else{url="/"+sitePath+"interface/comment.php?id="+id+"&page="+page;}
	ajax.get( 
		url,
		function(obj) {
			if (obj.responseText=="err"){
...
function diggVideo(id,div){
	ajax.get(
		"/"+sitePath+"duomiphp/ajax.php?id="+id+"&action=digg",
		function (obj){
...
function treadVideo(id,div){
	ajax.get(
		"/"+sitePath+"duomiphp/ajax.php?id="+id+"&action=tread",
		function (obj){
			var returnValue=Number(obj.responseText)
...
function diggNews(id,div){
	ajax.get("/"+sitePath+"duomiphp/ajax.php?id="+id+"&action=diggnews",function (obj){
			var returnValue=Number(obj.responseText)
			if (!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('(*^__^*) 嘻嘻……，顶得我真舒服！');}else if(obj.responseText=="err"){alert('顶失败')}else if(obj.responseText=="havescore"){alert('(*^__^*) 嘻嘻…… 这么热心啊，您已经顶过了！')}	
		}
	);
...
function treadNews(id,div){
	ajax.get("/"+sitePath+"duomiphp/ajax.php?id="+id+"&action=treadnews",function (obj){
			var returnValue=Number(obj.responseText)
			if(!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('小样儿，居然敢踩我！');}else if(obj.responseText=="err"){alert('踩失败')}	else if(obj.responseText=="havescore"){alert('我晕，您已经踩过了，想踩死我啊！')}	
		}
	);	
...
function getVideoHit(vid){
	ajax.get(
		"/"+sitePath+"duomiphp/ajax.php?action=hit&id="+vid,
		function (obj){
...
{
	ajax.get( 
		"/"+sitePath+"duomiphp/ajax.php?action=member",
		function (obj){
			var result=obj.responseText;
			set(document.getElementById('duomicms_member'),result);
		}
	);
}
...
function getNewsHit(nid){
	ajax.get(
		"/"+sitePath+"duomiphp/ajax.php?action=hitnews&id="+nid,
		function (obj){
			var result=obj.responseText
...
function markscore0(vd,d,t,s,l,ac){
	var alt=['很差','较差','还行','推荐','力荐'],url=ac=='news' ? ["/"+sitePath+"duomiphp/ajax.php?id="+vd+"&action=newsscore","/"+sitePath+"duomiphp/ajax.php?id="+vd+"&action=scorenews&score="] : ["/"+sitePath+"duomiphp/ajax.php?id="+vd+"&action=videoscore","/"+sitePath+"duomiphp/ajax.php?id="+vd+"&action=score&score="],
...
function showpf()
	{document.getElementById('duomicmsvpf1').style.display="none";document.getElementById('duomicmsvpf2').style.display="inline";}
function markscore1(vd,d,t,s,l,ac){
	var alt=['很差','较差','还行','推荐','力荐'],src=['/'+sitePath+'images/star0.gif','/'+sitePath+'images/star1.gif'],url=ac=='news' ? ["/"+sitePath+"duomiphp/ajax.php?id="+vd+"&action=newsscore","/"+sitePath+"duomiphp/ajax.php?id="+vd+"&action=scorenews&score="] : ["/"+sitePath+"duomiphp/ajax.php?id="+vd+"&action=videoscore","/"+sitePath+"duomiphp/ajax.php?id="+vd+"&action=score&score="],
	x=d,y=(Math.round(s / x * 100) / 100) || 0,id='STAR'+(new Date()).getTime();
	document.write('<span id="'+id+'" style="padding:5px">');
	
	document.write('<span id="duomicmsvpf1" onmouseover=showpf()></span>');
	document.write('<span id="duomicmsvpf2">');
	for(var i=1;i<=l;i++){
...
function markNews2(vid,style,len){
	...
	ajax.get(
		"/"+sitePath+"duomiphp/ajax.php?action=npingfen&id="+vid,
		function (obj){
			var result=obj.responseText;
			result=result.split(",");
			num=result[0];
			...
			if(style==1){
			//星星评分
			document.getElementById('duomicmsvpf2').style.display="none";
			id='STAR'+(new Date()).getTime();
			for(var ii=1;ii<=len;ii++){
			if(ii>sc){p=1;}else{p=0;}
			document.getElementById('duomicmsvpf1').innerHTML+='<img iid='+ii+' src="/images/star'+p+'.gif" style="cursor:pointer">';
			}
...
function markVideo2(vid,style,len){
	...
	ajax.get(
		"/"+sitePath+"duomiphp/ajax.php?action=vpingfen&id="+vid,
		function (obj){
			var result=obj.responseText;
			result=result.split(",");
			num=result[0];
...
function addRemoteFavor(){
	ajax.get(
		"/"+sitePath+"duomiphp/ajax.php?action=favorAjax&id="+play_vid+"&faction=add",
